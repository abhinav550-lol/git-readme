...
const CODE_LINES = [
	{ indent: 0, text: '# Fetching GitHub profile data...', color: 'text-zinc-500' },
	{ indent: 0, text: '', color: '' },
...
const Landing = () => {
	const { data: appUsersData, isLoading: isUsersLoading } = queries.useAppUsers()
	...
	useEffect(() => {
		...
		const observerCallback = (entries: IntersectionObserverEntry[], setter: (v: boolean) => void) => {
		...
		if (featureSectionRef.current) featuresObserver.observe(featureSectionRef.current)
		...
		<main className="relative min-h-[100dvh] overflow-hidden bg-zinc-950 text-white">
			...
			<section className="relative z-10 mx-auto grid min-h-[100dvh] w-full max-w-7xl grid-cols-1 items-center gap-8 px-6 py-20 md:grid-cols-2 md:gap-12 lg:px-12">
				...
				<div className="flex flex-col items-start justify-center">
					...
					{/* Headline */}
					<h1 className="landing-animate-fade-up landing-delay-1 mt-8 text-4xl font-semibold tracking-tighter text-white md:text-5xl lg:text-6xl" style={{ lineHeight: '1.05' }}>
						Generate GitHub<br />
						<span className="inline-block font-mono text-emerald-400" style={{ minWidth: '5ch' }}>
							{scrambledWord}
						</span><br />
					...
					<div className="landing-animate-fade-up landing-delay-4 mt-10 flex items-center gap-6 border-t border-white/6 pt-6">
						...
						<div className="flex flex-col">
							<span className="font-mono text-xl font-medium text-white">Live</span>
							<span className="text-xs text-zinc-600 uppercase tracking-wider">Data sync</span>
						</div>
						<div className="h-8 w-px bg-white/8" />
						<div className="flex flex-col">
							<span className="font-mono text-xl font-medium text-white">4</span>
							<span className="text-xs text-zinc-600 uppercase tracking-wider">Card types</span>
						</div>
					</div>
				</div>
				...
				<div className="landing-animate-fade-up landing-delay-3 relative hidden md:block">
					...
					>
						...
						<div className="flex items-center gap-2 border-b border-white/6 px-4 py-3">
							...
							<div className="h-3 w-3 rounded-full bg-zinc-700" />
							<div className="h-3 w-3 rounded-full bg-zinc-700" />
							<span className="ml-3 font-mono text-[0.65rem] text-zinc-600">profile-card-generator.yml</span>
						</div>
						...
						<div className="flex items-center gap-2 border-t border-white/6 px-5 py-3">
							...
							<span className="font-mono text-[0.7rem] text-zinc-500">generating profile cards...</span>
							<span className="h-4 w-[2px] bg-emerald-400" style={{ animation: 'landing-typing-cursor 1s step-end infinite' }} />
						</div>
					</div>

					{/* Floating decoration */}
					<div className="landing-animate-float absolute -right-6 -top-6 rounded-xl border border-white/8 bg-zinc-900/90 p-3 shadow-lg backdrop-blur-md" style={{ animationDelay: '1s' }}>
						<Sparkles className="h-5 w-5 text-emerald-400" />
					</div>
					<div className="landing-animate-float absolute -bottom-4 -left-4 rounded-xl border border-white/8 bg-zinc-900/90 px-3 py-2 shadow-lg backdrop-blur-md" style={{ animationDelay: '2.5s' }}>
						<span className="font-mono text-[0.7rem] text-emerald-400">cards.svg</span>
					</div>
				</div>
			</section>
			...
			<div className="relative z-10 overflow-hidden border-y border-white/[0.04] bg-zinc-950/80 py-4 backdrop-blur-md">
				<div className="landing-animate-marquee flex w-max items-center gap-10">
					{[
						'Profile Cards', 'Stats Cards', 'Language Breakdown', 'Contribution Graphs',
						'Real-time Data', 'Custom Themes', 'Dynamic Visuals', 'GitHub Auth',
						'Profile Cards', 'Stats Cards', 'Language Breakdown', 'Contribution Graphs',
						'Real-time Data', 'Custom Themes', 'Dynamic Visuals', 'GitHub Auth',
					].map((item, i) => (
						<span key={i} className="flex items-center gap-2 whitespace-nowrap text-sm text-zinc-600">
					...
					<div className="relative z-10 grid grid-cols-1 items-center gap-10 p-10 md:grid-cols-2 md:p-14">
						...
						<div>
							...
							<p className="mt-4 max-w-[160ch] text-md md:text-xl leading-relaxed text-zinc-500">
								Developers use GitREADME to generate profile cards and stats visualizations that make their GitHub stand out.
							</p>
